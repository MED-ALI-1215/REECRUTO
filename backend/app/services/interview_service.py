...
def _persist(db, *, service: str, prompt_version: str, prompt_tokens: int,
    ...
    try:
        ...
        record_ai_call(
            db, service=service, model=get_settings().GROQ_MODEL,
...
def generate_questions(job_title: str, job_description: str, n: int = 5, db=None) -> list[str]:
    ...
    prompt = load_prompt("question_generation_v1.txt",
                         n=n, job_title=job_title, job_description=job_description)
    t0 = time.monotonic()
    try:
        resp = _groq_client().chat.completions.create(
            messages=[
                {"role": "system", "content": "You are an expert technical interviewer. Return only the numbered list."},
                {"role": "user", "content": prompt},
            ],
            model=settings.GROQ_MODEL, temperature=0.7, max_tokens=800, timeout=30,
        )
        latency = (time.monotonic() - t0) * 1000
        log_ai_call(logger, service="question_generation", model=settings.GROQ_MODEL,
                    prompt_tokens=resp.usage.prompt_tokens, latency_ms=latency, success=True)
        _persist(db, service="question_generation", prompt_version="question_generation_v1",
...
def score_answer(question: str, answer: str, job_title: str, job_description: str, db=None) -> dict:
    ...
    settings = get_settings()
    prompt = load_prompt("answer_scoring_v1.txt",
                         job_title=job_title,
    ...
    try:
        resp = _groq_client().chat.completions.create(
            ...
            model=settings.GROQ_MODEL, temperature=0.3, max_tokens=400, timeout=30,
        )
        latency = (time.monotonic() - t0) * 1000
        log_ai_call(logger, service="answer_scoring", model=settings.GROQ_MODEL,
                    prompt_tokens=resp.usage.prompt_tokens, latency_ms=latency, success=True)
        _persist(db, service="answer_scoring", prompt_version="answer_scoring_v1",
                 prompt_tokens=resp.usage.prompt_tokens,
                 completion_tokens=resp.usage.completion_tokens,
                 latency_ms=latency, success=True)
...
def generate_final_report(candidate_name: str, job_title: str,
                           questions: list, answers: list, scores: list, db=None) -> dict:
    settings = get_settings()
    qa_summary = "\n".join(
        f"Q{i+1}: {q}\nA: {a}\nScore: {s.get('score', 0)}/100"
        for i, (q, a, s) in enumerate(zip(questions, answers, scores))
    )
    prompt = load_prompt("final_report_v1.txt",
                         job_title=job_title,
    ...
    try:
        resp = _groq_client().chat.completions.create(
            messages=[
                {"role": "system", "content": "You are a senior hiring manager. Respond only with valid JSON."},
                {"role": "user", "content": prompt},
            ],
            model=settings.GROQ_MODEL, temperature=0.3, max_tokens=600, timeout=30,
        )
        latency = (time.monotonic() - t0) * 1000
        log_ai_call(logger, service="final_report", model=settings.GROQ_MODEL,
                    prompt_tokens=resp.usage.prompt_tokens, latency_ms=latency, success=True)
        _persist(db, service="final_report", prompt_version="final_report_v1",
                 prompt_tokens=resp.usage.prompt_tokens,
                 completion_tokens=resp.usage.completion_tokens,
                 latency_ms=latency, success=True)

        result = _parse_json_response(resp.choices[0].message.content.strip())
        logger.info("Final report: candidate=%s score=%s recommendation=%s",
                    candidate_name, result.get("overall_score"), result.get("recommendation"))
        return result
